package us.rettopvp.cookie.util.scoreboard.entry;
...
public class BoardEntry {
    ...
    public BoardEntry send(int position) {
        ...
        if (this.text.length() > 16) {
            ...
            boolean addOne = this.team.getPrefix().endsWith("§");
            if (addOne) {
                this.team.setPrefix(this.text.substring(0, 15));
            }
            String suffix = ChatColor.getLastColors(this.team.getPrefix()) + this.text.substring(addOne ? 15 : 16, this.text.length());
            if (suffix.length() > 16) {
