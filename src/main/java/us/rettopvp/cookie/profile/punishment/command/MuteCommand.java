...
public class MuteCommand extends CustomCommand {
	...
	public MuteCommand() {
		super("mute", "Silence the chat.", new String[] {"mc", "chatmute"});
	}
	
	@Override
	public boolean onCommand(CommandSender commandsender, Command command, String label, String[] arguments) {
		if(!commandsender.hasPermission("cookie.command.mute")) {
		...
		if(profile.getActiveMute() != null) {
			Messager.sendMessage(commandsender, "&cThat player is already muted.");
			return true;
		}
		...
		if(player != null) {
			Messager.sendMessage(player, "&cYou have been muted by " + staffName + " &cfor: " + reason);
			if(!punishment.isPermanent()) {
				Messager.sendMessage(player, "&cThis mite will expire in &f" + punishment.getTimeRemaining());
			}
		}
